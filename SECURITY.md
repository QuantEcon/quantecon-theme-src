...
| `ws@^8.0.0` | `^8.20.1`  | GHSA-58qx-3vcg-4xpx — scoped to the 8.x copies (Jupyter/jsdom test chain) so the `ws@7` copy (thebe) is untouched; `ws@7` itself is now clean (7.5.12 ≥ the last patched 7.5.11 via #103, no open alerts) |
...
| `brace-expansion@^5.0.0` | `^5.0.9`  | same advisory family, 5.x copy (dev chain). Floor is 5.0.9, not 5.0.7: the GitHub advisory DB patches the 5.x line at 5.0.8 (GHSA-mh99) and 5.0.9 (GHSA-rgw5) even though this repo's Dependabot alerts only surface the 1.x ranges for those two |
...
| `@remix-run/react`, `@remix-run/server-runtime`, `@remix-run/router`, `react-router` | high/med | Patched only in Remix v2 / react-router 6.30+ or 7.18+ (GHSA-2j2x-hqr9-3h42, the `//` protocol-relative open redirect, is fixed in 6.30.4; GHSA-337j-9hxr-rhxg and GHSA-wrjc-x8rr-h8h6 have **no 6.x fix at all** — first patched in react-router 7.18.0). `@remix-run/react@1.17` exact-pins `react-router-dom@6.13.0` + `@remix-run/router@1.6.3`, which ship paired internal APIs, so the routers can't be pulled forward independently. Deferred with the v1 pin. |
...
it moves when `markdown-it` does), `nanoid` (4→5), `tar` (6→7,
...
GHSA-qffp-2rhf-9h96 / GHSA-9ppj-qmqm-q256 / GHSA-vmf3-w455-68vh /
GHSA-w8wr-v893-vjvp — all fixed only in 7.5.x),
