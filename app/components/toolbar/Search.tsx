/* Forked from @myst-theme/site@1.3.0 (src/components/Navigation/Search.tsx).
   Local changes: the dialog trigger is an icon-only toolbar button instead of
   the boxed placeholder input, and the keyboard-shortcut hint (⌘K / CTRL K)
   renders inside the dialog over the right edge of the search input. The
   internals (SearchForm, SearchResults, useSearch) are not exported upstream,
   hence the full copy. */
import {
  createElement,
  useEffect,
        ...
        <div className="relative flex w-full h-10 flow-row gap-x-1 ">
          ...
          <input
            ...
            className={classNames(
              ...
              // Reserve room for the shortcut hint overlaid on the right (sm+ only)
              'sm:pe-24',
...
const SearchTriggerButton = forwardRef<
  HTMLButtonElement,
  SearchTriggerButtonProps & Dialog.DialogTriggerProps
>(({ className, disabled, ...props }, ref) => {
  ...
  return (
    <button
      ...
      aria-label="Search"
      className={classNames('myst-search-bar flex items-center cursor-pointer', className)}
      disabled={!!disabled}
      ref={ref}
    >
